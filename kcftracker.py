...
def fftd(img, backwards=False):
    ...
    return cv2.dft(np.float32(img),
                   flags=((cv2.DFT_INVERSE | cv2.DFT_SCALE) if backwards else
                          cv2.DFT_COMPLEX_OUTPUT))  # 'flags =' is necessary!
...
def complexDivision(a, b):
    ...
    res[:, :,
        0] = (a[:, :, 0] * b[:, :, 0] + a[:, :, 1] * b[:, :, 1]) * divisor
    res[:, :,
        1] = (a[:, :, 1] * b[:, :, 0] + a[:, :, 0] * b[:, :, 1]) * divisor
    return res
...
def rearrange(img):
    ...
    img_[0:yh, 0:xh], img_[yh:img.shape[0],
                           xh:img.shape[1]] = img[yh:img.shape[0],
                                                  xh:img.shape[1]], img[0:yh,
                                                                        0:xh]
    img_[0:yh, xh:img.shape[1]], img_[yh:img.shape[0],
                                      0:xh] = img[yh:img.shape[0],
                                                  0:xh], img[0:yh,
                                                             xh:img.shape[1]]
    return img_
...
class KCFTracker:
    ...
    def getFeatures(self, image, init_hann: bool, scale_adjust: float=1.0):
        """
        
        Arguments:
            image: ndarray;
            init_hann: whether init hann;
            scale_adjust: ;
        Returns:
    ...
    def detect(self, z, x):
        ...
        if (pi[1] > 0 and pi[1] < res.shape[0] - 1):
            p[1] += self.subPixelPeak(res[pi[1] - 1, pi[0]], pv, res[pi[1] + 1,
                                                                     pi[0]])
    ...
    def train(self, x, train_interp_factor):
        ...
        self._tmpl = (
            1 - train_interp_factor) * self._tmpl + train_interp_factor * x
        self._alphaf = (1 - train_interp_factor
                        ) * self._alphaf + train_interp_factor * alphaf

    def init(self, roi, image) -> None:
        """using roi init tracker.
    ...
    def update(self, image):
        ...
        if (self.scale_step != 1):
            ...
            if (self.scale_weight * new_peak_value1 > peak_value
                    and new_peak_value1 > new_peak_value2):
                loc = new_loc1
        ...
        self._roi[0] = cx - self._roi[2] / 2.0 + loc[
            0] * self.cell_size * self._scale
        self._roi[1] = cy - self._roi[3] / 2.0 + loc[
            1] * self.cell_size * self._scale
